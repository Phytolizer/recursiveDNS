...
cStringSpan winsock::winsock_download(cStringSpan host, cStringSpan dnsaddr) {
    ...
    printf("%-10s: %-15s\n", "Lookup", host.string);
    in_addr_t IP = inet_addr(host.string);
    ...
    if (s == -1) {
        printf("\tsocket() generated error: %s\n", strerror(errno));
    ...
    dest.sin_addr.s_addr = inet_addr(dnsaddr.string);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (sendto(s, packet, packetSize, 0, (sockaddr*)&dest, sizeof(dest)) == -1) {
            printf("\tsendto generated error: %s\n", strerror(errno));
            return cStringSpan(nullptr, 0);
        }
        begin = clock();
        int destlen = sizeof(dest);
        struct timeval recvTimeout = {TIMEOUT_MS / 1000, (TIMEOUT_MS % 1000) * 1000};
        setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (const char*)&recvTimeout, sizeof(timeval));
        bytesRecieved =
                recvfrom(s, response, MAX_DNS_SIZE, 0, (sockaddr*)&dest, (socklen_t*)&destlen);
        end = clock();

        if (bytesRecieved == -1) {
            if ((end - begin) > 10000) {
                printf("timeout in %lu ms\n", end - begin);
            } else {
    ...
    printf("response in %zu ms with %d bytes\n", end - begin, bytesRecieved);
    ...
    printf("\tTXID: 0x%.4X flags 0x%.4x questions %d anwsers %d authority %d additional %d\n",
            ntohs(dh->ID), ntohs(dh->flags), ntohs(dh->nQuestions), ntohs(dh->nAnswers),
            ntohs(dh->nAuthority), ntohs(dh->nAdditional));
    // TODO: below + timeout 3x attempts stuff
    // if no timeout and we get a response
    // validate txid
    if (dh->ID != htons(getpid())) {
    ...
    u_int minPktSize = sizeof(FixedDNSHeader) + ntohs(dh->nQuestions) * sizeof(QueryHeader) +
            sizeof(DNSAnswerHeader) *
                    (ntohs(dh->nAnswers) + ntohs(dh->nAuthority) + ntohs(dh->nAdditional));
    ...
    RR* answers = new RR[ntohs(dh->nAnswers)]{};
    ...
    for (int i = 0; i < ntohs(dh->nQuestions); i++) {
        ...
        questions[i].name =
                parseName((unsigned char*)curr, (unsigned char*)response, &count, bytesRecieved);
        curr += count;
    ...
    // read anwsers
    int nAnswers = ntohs(dh->nAnswers);
    for (int i = 0; i < nAnswers; i++) {
        ...
        answers[i].name =
                parseName((unsigned char*)curr, (unsigned char*)response, &count, bytesRecieved);
        curr += count;
        answers[i].header = (DNSAnswerHeader*)curr;
        curr += sizeof(DNSAnswerHeader);
        // check for truncated DNSAnwserHeader
        if ((curr - response) > bytesRecieved) {
            printf("\t++ invalid record: truncated RR anwser header\n");
            cleanAndExit(s);
        ...
        // printf("Header type: %hu class: %hu ttl: %du len: %hu\n", ntohs(anwsers[i].header->type),
        // ntohs(anwsers[i].header->aClass), ntohs(anwsers[i].header->ttl),
        // ntohs(anwsers[i].header->len));
        if (ntohs(answers[i].header->type) == 1) { // ip
            // ipv4
            // extract ip address here
            answers[i].record = new unsigned char[4];
            for (int j = 0; j < 4; j++) {
                answers[i].record[j] = curr[j];
            }
            curr += 4;
        } else {
            answers[i].record = parseName(
                    (unsigned char*)curr, (unsigned char*)response, &count, bytesRecieved);
            curr += count;
        }
    }
    // read authority
    ...
    for (int i = 0; i < nAuthority; i++) {
        ...
        authority[i].name =
                parseName((unsigned char*)curr, (unsigned char*)response, &count, bytesRecieved);
        curr += count;
        authority[i].header = (DNSAnswerHeader*)curr;
        curr += sizeof(DNSAnswerHeader);
        // check for truncated DNSAnwserHeader
        if ((curr - response) > bytesRecieved) {
            printf("\t++ invalid record: truncated RR anwser header\n");
    ...
    for (int i = 0; i < nAdditional; i++) {
        ...
        additional[i].name =
                parseName((unsigned char*)curr, (unsigned char*)response, &count, bytesRecieved);
        curr += count;
        additional[i].header = (DNSAnswerHeader*)curr;
        curr += sizeof(DNSAnswerHeader);
        // check for truncated DNSAnswerHeader
        if ((curr - response) > bytesRecieved) {
            printf("\t++ invalid record: truncated RR anwser header\n");
            cleanAndExit(s);
            exit(-1);
    ...
    printf("\t------------ [questions] ----------\n");
    for (int i = 0; i < ntohs(dh->nQuestions); i++) {
        printf("\t\t%s type %hu class %hu\n", (char*)questions[i].name,
                ntohs(questions[i].header->type), ntohs(questions[i].header->qClass));
    }
    // output anwsers
    printf("\t------------ [anwsers] ------------\n");
    for (int i = 0; i < ntohs(dh->nAnswers); i++) {
        if (ntohs(answers[i].header->type) == 1)
            printf("\t\t%s %s %hhu.%hhu.%hhu.%hhu TTL = %" PRIu32 "\n", answers[i].name,
                    typeToString(ntohs(answers[i].header->type)).c_str(), answers[i].record[0],
                    answers[i].record[1], answers[i].record[2], answers[i].record[3],
                    ntohl(answers[i].header->ttl));
        else
            printf("\t\t%s %s %s TTL = %" PRIu32 "\n", answers[i].name,
                    typeToString(ntohs(answers[i].header->type)).c_str(), answers[i].record,
                    ntohl(answers[i].header->ttl));
    }
    // output authority
    printf("\t------------ [authority] ----------\n");
    for (int i = 0; i < ntohs(dh->nAuthority); i++) {
        if (ntohs(authority[i].header->type) == 1)
            printf("\t\t%s %s %hhu.%hhu.%hhu.%hhu TTL = %" PRIu32 "\n", authority[i].name,
                    typeToString(ntohs(authority[i].header->type)).c_str(), authority[i].record[0],
                    authority[i].record[1], authority[i].record[2], authority[i].record[3],
                    ntohl(authority[i].header->ttl));
        else
            printf("\t\t%s %s %s TTL = %" PRIu32 "\n", authority[i].name,
                    typeToString(ntohs(authority[i].header->type)).c_str(), authority[i].record,
                    ntohl(authority[i].header->ttl));
    }
    // output additional
    printf("\t------------ [additional] ---------\n");
    for (int i = 0; i < ntohs(dh->nAdditional); i++) {
        if (ntohs(additional[i].header->type) == 1)
            printf("\t\t%s %s %hhu.%hhu.%hhu.%hhu TTL = %" PRIu32 "\n", additional[i].name,
                    typeToString(ntohs(additional[i].header->type)).c_str(),
                    additional[i].record[0], additional[i].record[1], additional[i].record[2],
                    additional[i].record[3], ntohl(additional[i].header->ttl));
        else
            printf("\t\t%s %s %s TTL = %" PRIu32 "\n", additional[i].name,
                    typeToString(ntohs(additional[i].header->type)).c_str(), additional[i].record,
                    ntohl(additional[i].header->ttl));
    }
...
void winsock::cleanAndExit(int s) {
    close(s);
}
